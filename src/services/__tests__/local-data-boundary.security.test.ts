...
// Test liegt in src/services/__tests__/ → drei Ebenen hoch zur Repo-Wurzel,
// dann in src/services. fileURLToPath (nicht new URL().pathname) liefert unter
// vitest den korrekten Dateisystempfad.
const SERVICES_DIR = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
...
describe('[PRIVACY] lokale Finanzdaten-Grenze', () => {
  it.each(LOCAL_ONLY_SERVICES)('%s besitzt keinen Supabase-Datenpfad', (fileName) => {
    const source = readFileSync(path.join(SERVICES_DIR, fileName), 'utf8');
...
describe('[PRIVACY] Cloud-Ausnahme (MCP POC) bleibt aggregat-only', () => {
  const source = readFileSync(path.join(SERVICES_DIR, CLOUD_EXCEPTION), 'utf8');
