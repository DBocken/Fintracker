...
/**
 * Depot-Kennzahlen. Lädt Depot und Positionen; gerechnet wird in der reinen
 * `summarizePortfolio` (`features/trading/domain/portfolio-summary.ts`).
 */
export async function getPortfolioSummary(portfolioId: string): Promise<PortfolioSummary> {
  const portfolio = await getPortfolioById(portfolioId);
  if (!portfolio) throw new Error(t('portfolio.notFound'));
  return summarizePortfolio(portfolio, await getPositions(portfolioId));
}
