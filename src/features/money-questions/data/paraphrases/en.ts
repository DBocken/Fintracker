/**
 * English paraphrases — smaller than the German set, and that asymmetry is a
 * NAMED decision, not neglect: the measured corpus (the 225 owner-provided
 * questions) is German, so German carries the calibration. English grows the
 * same way German did — through curation and the local learning loop.
 */
import { LUECKE_KLASSE } from '@/lib/question-intent-model';

export const PARAPHRASEN_EN: Readonly<Record<string, readonly string[]>> = {
  'ausgaben.haendler': [
    'how much did i spend at amazon',
    'my total payments to netflix',
    'what did i pay at the supermarket',
  ],
  'ausgaben.kategorie': [
    'how much went on groceries',
    'my spending on clothes this year',
    'what did i pay for leisure',
  ],
  'ausgaben.gesamt': [
    'my total spending last month',
    'how much money went out overall',
    'sum of all my expenses this year',
  ],
  'ausgaben.topHaendler': [
    'which merchants get most of my money',
    'my most expensive stores',
    'top ten merchants by spending',
  ],
  'ausgaben.topKategorien': [
    'what do i mostly spend money on',
    'my biggest spending categories',
    'where does most of my money go',
  ],
  'ausgaben.ungewoehnlich': [
    'any unusual spending lately',
    'which months were oddly expensive',
    'outliers in my expenses',
  ],
  'einnahmen.zeitraum': [
    'how much came in this month',
    'my income this quarter',
    'total earnings this year',
  ],
  'einkommen.letztes': [
    'when did my last salary arrive',
    'how much was my last paycheck',
    'has my salary come in yet',
  ],
  'einkommen.durchschnitt': [
    'my average monthly income',
    'what do i earn on average',
    'was my salary lower than usual',
  ],
  'einkommen.schwankung': [
    'how stable is my income',
    'does my income vary a lot',
    'how much does my pay fluctuate',
  ],
  'vertrag.jahreskosten': [
    'what does netflix cost me per year',
    'yearly cost of my phone contract',
    'my insurance per year',
  ],
  'abos.liste': [
    'show me my subscriptions',
    'which contracts am i running',
    'list of my active subscriptions',
  ],
  'abos.summe': [
    'what do all my subscriptions cost together',
    'monthly total of my subscriptions',
    'my subscriptions combined',
  ],
  'vertraege.teurer': [
    'which subscriptions got more expensive',
    'any price increases in my contracts',
    'which contracts cost more now',
  ],
  'fixkosten.monatlich': [
    'how high are my fixed costs',
    'my fixed monthly expenses',
    'what do i pay fixed each month',
  ],
  'fixkosten.anteil': [
    'what share of my income is fixed costs',
    'how much of my salary is already committed',
    'fixed cost ratio of my income',
  ],
  'schulden.restschuld': [
    'how much do i still owe',
    'my outstanding debt',
    'current state of my loans',
  ],
  'raten.offen': [
    'how many instalments are left',
    'remaining payments on my financing',
    'when is my instalment plan done',
  ],
  'leistbarkeit.anschaffung': [
    'can i afford a 3000 vacation',
    'is a 1500 purchase feasible for me',
    'can i buy a laptop for 1200',
  ],
  'budget.aktion': [
    'create a budget of 200 for groceries',
    'set my leisure budget to 150',
    'increase the shopping budget by 40',
    'delete the budget for cinema',
  ],
  'szenario.kombination': [
    'what if i sell my car and take the train instead',
    'run the numbers on a raise plus higher rent',
    'play through me quitting and starting something new',
  ],
  'ausgaben.durchschnitt': [
    'what does that cost me each month on average',
    'monthly burden from these costs',
    'average monthly spend on that',
  ],
  'ausgaben.anteil': [
    'how big a slice of everything is that',
    'what portion of my costs goes there',
    'what percentage of the whole is that',
  ],
  'ausgaben.jeVorgang': [
    'how pricey is one shop there on average',
    'what do i drop per visit',
    'typical size of a single payment there',
  ],
  'ausgaben.extremwert': [
    'when did i pay the most for that',
    'which month was the priciest',
    'what was my single biggest expense',
  ],
  'ausgaben.trend': [
    'is that getting more or less over time',
    'is it going up or down',
    'how does that run across the months',
  ],
  'vergleich.haendler': [
    'which of the two shops costs me more',
    'put the two merchants side by side',
    'who is pricier of the two',
  ],
  'vergleich.kategorie': [
    'which of the two areas costs me more',
    'put the two areas side by side',
    'which pot is bigger',
  ],
  'vergleich.zeitraum': [
    'has that grown since back then',
    'do i pay more today than before for that',
    'difference to the same period before',
  ],
  'budget.status': [
    'how are my budgets doing',
    'am i over budget',
    'which budgets are blown',
  ],
  'budget.rest': [
    'how much budget is left',
    'what remains in my grocery budget',
    'remaining budget this month',
  ],
  'budget.tagesrate': [
    'how much can i spend per day',
    'my daily budget until month end',
    'daily allowance so the budget lasts',
  ],
  'forecast.monatsende': [
    'where will my balance land at month end',
    'will my money last until the end of the month',
    'projected balance end of month',
  ],
  'forecast.horizont': [
    'my balance in three months',
    'where am i financially in half a year',
    'when will my account run low',
  ],
  'konto.saldo': [
    'how much is in my current account',
    'what is the balance of my savings account',
    'how much money do i have on that account',
    'show me the balance of my checking account',
    'what is on my credit card',
    'balance of my savings account please',
    'how much is left on the account',
    'what does that account hold right now',
  ],
  'konto.gesamt': [
    'how much money do i have in total',
    'what is on all my accounts together',
    'total balance across my accounts',
    'how much cash do i have altogether',
    'sum of all my account balances',
    'how much liquid money do i have right now',
    'what do all my accounts add up to',
    'how much do i own in cash and balances',
  ],
  'depot.wert': [
    'how much is my portfolio worth',
    'what is the total in my shares',
    'value of my securities',
    'how much money is in the portfolio',
    'what are my investments worth',
    'current portfolio value',
    'how large is my portfolio',
    'how much do i hold in securities',
  ],
  'depot.rendite': [
    'how is my portfolio doing',
    'am i up on my shares',
    'how much gain do i have in the portfolio',
    'am i losing money on my securities',
    'return on my investments',
    'how much have i made with shares',
    'is my portfolio down',
    'gain or loss in the portfolio',
  ],
  'depot.positionen': [
    'which shares do i own',
    'what is in my portfolio',
    'my largest positions',
    'which securities do i hold',
    'show me the positions in the portfolio',
    'which titles are in my depot',
    'what is my biggest holding',
    'list of my shares',
  ],
  'vermoegen.gesamt': [
    'what am i worth',
    'how large is my net worth',
    'what do i have on balance',
    'how much wealth do i have',
    'what is left after debts',
    'my total net worth',
    'how rich am i',
    'how much do i own net',
  ],
  'vermoegen.aufteilung': [
    'what is my net worth made of',
    'how is my wealth composed',
    'how is my net worth split',
    'how much of it is cash and how much invested',
    'composition of what i own',
    'split between accounts and portfolios',
    'how is my money distributed',
    'what makes up my net worth',
  ],
  'anlass.kosten': [
    'what did my holiday cost me',
    'how expensive was the wedding',
    'cost of my move',
    'how much did the project cost',
    'what did the trip cost in total',
    'total for my event',
    'how much money went into the holiday',
    'total cost of my wedding',
  ],
  'anlass.liste': [
    'which events do i have',
    'show me my events',
    'which event was the most expensive',
    'overview of my projects',
    'what events have i created',
    'my events with their cost',
    'list of my occasions',
    'which projects cost me most',
  ],
  'anlass.vorschlag': [
    'which transactions still belong to the holiday',
    'what is still missing for my wedding',
    'are there more transactions for the move',
    'find matching transactions for the project',
    'what else could belong to the event',
    'which expenses are not assigned to the holiday yet',
    'transaction suggestions for my trip',
    'what is still open on the event',
  ],
  'transfer.kandidaten': [
    'do i have transfers that are not recognised',
    'are there transfers between my accounts',
    'which transactions are actually internal transfers',
    'find unrecognised transfers',
    'show me possible internal transfers',
    'which pairs are probably transfers',
    'were transfers counted as expenses by mistake',
    'search for unlinked transfers',
  ],
  'steuer.gewinn': [
    'how much profit did i make this year',
    'what is left after business expenses',
    'my surplus from self-employment',
    'how much did i earn self-employed',
    'how do my business accounts look',
    'profit or loss in the business',
    'what did i earn with the business on balance',
    'how high is my business profit',
  ],
  'steuer.ruecklage': [
    'how much do i have to set aside for taxes',
    'is my tax reserve enough',
    'what is still missing for the tax office',
    'do i have enough put aside for tax',
    'how much should i save for the tax office',
    'check my tax reserve',
    'what do i still need to put aside for tax',
    'is my tax pot full enough',
  ],
  'ziel.obergrenze': [
    'how expensive may my holiday get at most',
    'what can i spend at most without going under',
    'how high may the purchase be for the buffer to hold',
    'upper limit for my next bigger expense',
    'up to what amount am i still fine',
    'how much can i afford at most',
    'what is the maximum i can spend',
    'how much may the new bike cost without it getting tight',
  ],
  'ziel.sparrate': [
    'how much do i have to save monthly for 5000',
    'how much to set aside each month for the holiday',
    'what savings rate do i need for the purchase',
    'how much to put aside to manage 3000',
    'monthly amount so that i reach the goal',
    'how much to save per month for a new car',
    'what amount must i put away monthly',
    'how high would my savings rate have to be',
  ],
  'liquiditaet.reichweite': [
    'how long will my money last',
    'how many months can i get by',
    'how long do i last on this balance',
    'will my savings last half a year',
    'how long could i live without income',
    'how many months of cushion do i have',
    'how far do i get with what is there',
    'how long does my balance carry me',
  ],
  'abbuchung.naechste': [
    'what will be debited soon',
    'which debits are still coming',
    'what leaves my account in the next weeks',
    'show me the next due items',
    'which direct debits are due',
    'what do i still have to pay this month',
    'show upcoming debits',
    'what is going to be taken from me soon',
  ],
  'abbuchung.letzte': [
    'when was i last at rewe',
    'when did i last shop at aldi',
    'last purchase at dm',
    'when was the last time at lidl',
    'when did i last pay netflix',
    'last time amazon when was that',
    'when is the last transaction from spotify',
    'when was my last visit to edeka',
  ],
  'einkommen.arten': [
    'where does my money come from',
    'what income do i actually have',
    'what is my income made of',
    'what sources of income do i have',
    'how is my income composed',
    'what all flows in for me',
    'breakdown of my income',
    'what kinds of income do i have',
  ],
  'schulden.dauer': [
    'how long will i keep paying my debts',
    'when am i finally debt free',
    'how many months until it is paid off',
    'when will i have paid everything back',
    'term until my debts are gone',
    'how long does this go on with the instalments',
    'when am i done paying it off',
    'how many years do i keep paying',
  ],
  'schulden.zinsen': [
    'how much interest do i pay in total',
    'what do my debts cost me in interest',
    'how high is my interest burden until the end',
    'how much goes to interest',
    'total interest until the last instalment',
    'what do i pay on top in interest',
    'how expensive are my loans really',
    'total interest cost of my debts',
  ],
  'schulden.sondertilgung': [
    'what does it do if i pay 100 more',
    'how much do i save with an extra repayment of 200',
    'is it worth paying 50 more a month',
    'what happens if i pay 150 extra monthly',
    'how much earlier am i done with 100 more',
    'extra repayment 300 what does that do',
    'if i add 80 a month how does that help',
    'interest saved with 250 more per month',
  ],
  'kategorie.begruendung': [
    'why is the rewe transaction in groceries',
    'why was amazon filed like that',
    'why does dm end up in drugstore',
    'how come netflix sits in leisure',
    'why is the aldi transaction in there',
    'why does it assign lidl that way',
    'why is spotify in this category',
    'how does the assignment at edeka come about',
  ],
  'kategorie.aktion': [
    'assign the rewe transactions to groceries',
    'file all dm transactions under drugstore',
    'remember that aldi is always groceries',
    'categorise netflix as leisure',
    'put the petrol station into mobility',
    'always assign amazon to shopping from now on',
    'move the lidl transactions into groceries',
    'remember spotify belongs to leisure',
  ],
  'verfuegbar.bisGehalt': [
    'what can i still spend until payday',
    'how much is free until my next salary',
    'will i make it to payday',
  ],
  [LUECKE_KLASSE]: [
    'how do i get rich quickly',
    'what is the best way to save',
    'which investment do you recommend',
    'what would you do in my place',
    'how should i split money with my partner',
    'how much house can i afford',
    'what should my day rate be as a freelancer',
    'what if i quit my job',
    'is buying better than renting for me',
    'which insurance do i really need',
  ],
};
