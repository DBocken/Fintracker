/**
 * Erkennung möglicher interner Überträge aus Betrag, Konto und Datum — rein,
 * ohne I/O.
 *
 * Lag bis Welle 2 im `transfer-service`, wo sie zuerst gebraucht wurde. Der
 * Chat beantwortet seither „Habe ich Umbuchungen, die nicht als solche
 * erkannt sind?", und ein Registereintrag darf `src/services/` nicht
 * importieren — nachgebaut wäre die Heuristik ein zweiter Ort, an dem
 * Toleranz und Zeitfenster auseinanderlaufen können. Das I/O bleibt beim
 * Dienst (`markTransferPair`), die Erkennung steht hier.
 */
...
/**
 * Toleranz und Zeitfenster stehen EINMAL hier und werden exportiert. Der
 * IBAN-gestützte Planer im Dienst prüft dieselben zwei Bedingungen; zwei
 * Zahlenpaare für eine Regel wären zwei Orte, an denen sie auseinanderlaufen.
 */
