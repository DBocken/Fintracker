      ...
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              {t('dashboard.balanceTitle', 'Kontostand')}
            </div>
          ...
          <dl className="grid grid-cols-2 gap-x-8 gap-y-4 sm:grid-cols-4 lg:text-right">
            ...
            <div>
              <dt className="flex items-center gap-1 text-xs text-muted-foreground lg:justify-end">
                ...
                {t('dashboard.expenses', 'Ausgaben')}
              </dt>
              <dd className="mt-1 text-lg font-semibold">{gentleModeEnabled ? '***' : eur.format(expenses)}</dd>
            </div>
            <div>
              <dt className="text-xs text-muted-foreground">{t('dashboard.balance', 'Saldo')}</dt>
              <dd
            ...
            <div>
              <dt className="text-xs text-muted-foreground">{t('dashboard.transactions', 'Transaktionen')}</dt>
              <dd className="mt-1 text-lg font-semibold">
                {count}
                <span className="text-sm font-normal text-muted-foreground"> {t('dashboard.of', 'von')} {totalTransactions}</span>
              </dd>
            </div>
          </dl>
        </div>
