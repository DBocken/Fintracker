...
| **F-SEC-3** | `supabase/functions/refresh-balances/index.ts:158` | Rate-Limit-Zähler in `balance_refresh_limits` per RLS **vom Nutzer selbst zurücksetzbar** → wirkungslos | Zähler mit `service_role` schreiben, Nutzer nur SELECT; oder SECURITY-DEFINER-RPC | PLAUSIBLE · ✅ **behoben 2026-08-06** (SECURITY-DEFINER-RPC, Migration `20260806120000`; zusätzlich TOCTOU zwischen Prüfen und Hochzählen geschlossen) |
