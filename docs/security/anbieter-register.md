...
| `huggingface.co`, `*.hf.co` | Hugging Face SAS | **FR (EU)** — CDN-Edges global, gemessen u. a. `us.aws.cdn.hf.co` (AWS CloudFront, US); die Auslieferung kann also über US-Infrastruktur laufen | Datenquelle (Modell-CDN, **Opt-in**) | Einmaliger Download **statischer Modelldateien** (multilingual-e5-small, int8, ~135 MB) für die lokale Frage-Zuordnung (Router-Stufe 3, Welle S). Läuft NUR nach ausdrücklicher Einwilligung auf der Fragen-Fläche; der Request enthält **keine Nutzerdaten** — keine Frage, keine Buchung, keine Kennung; übertragen wird die IP des Geräts wie bei jedem Download. Die Inferenz selbst läuft ausschliesslich auf dem Gerät; danach kommt das Modell aus dem Browser-Cache | entfällt (kein Personenbezug im Inhalt; IP-Übertragung wie bei jedem CDN — im Datenschutztext zu nennen → WP 6.1) | aktiv (Opt-in) | 2026-08-28 |
...
| `cdn.jsdelivr.net` | jsDelivr | onnxruntime-web (via `@huggingface/transformers`) lädt seine WASM-Laufzeit per VORGABE von dort nach, wenn `wasmPaths` nicht gesetzt ist; CSP-blockiert — Router-Stufe 3 produktiv defekt trotz installiertem Modell (Nutzerfund 28.08.) | **Behoben durch Selbst-Hosting**: `wasmPaths = '/ort/'` (`semantic-intent-service.ts`), Dateien aus exakt der gepinnten Version via Vite-Plugin (`scripts/ort-laufzeit-core.mjs`) — es fliesst nichts mehr dorthin, die URL steht nur noch ungenutzt in der Bibliotheksvorgabe |
