...
| `pnpm check:transaction-limits` | Verbietet ein **numerisches Literal** als Limit von `getTransactions(`. Die Liste ist datum-absteigend sortiert und wird abgeschnitten — rund 45 Aufrufstellen wählten dafür eine Zahl (500…10000), und **keine** prüfte, ob sie gegriffen hat. Ein Ausschnitt sieht aber aus wie ein Bestand: Der Klassifikator trainiert auf 1.000 Buchungen, die Vertragserkennung sieht keinen Jahresvertrag, die Dubletten-Prüfung findet genau die Dublette nicht, wegen der sie läuft, und die Steuersumme ist schlicht falsch. Nichts davon wird rot. Ersatz ist `getAllTransactions()` für Auswertungen und `getTransactionsPage(limit, offset)` für echte Seiten — dort **ist** das Limit die Aussage. Geprüft werden `services`, `hooks`, **`components`, `pages` und `features`**: Der Plan des Audits nannte nur die ersten beiden, nachgemessen lag die Hälfte der Aufrufer in der Oberfläche, darunter Steuerbericht und EÜR — ein Wächter, der dort nicht hinsieht, wo die Steuersumme entsteht, lässt den teuersten Fall zurückkommen (dieselbe Lehre wie bei `check:money-format`). Ein durchgereichtes oder berechnetes Limit bleibt unangetastet: Wer eine Variable übergibt, hat sie anderswo begründet. **Ohne Ausnahmeliste** — ein Eintrag hiesse „an dieser Stelle darf die Summe falsch sein". Erkennung in `transaction-limits-core.mjs`, ohne Dateisystem testbar. Läuft in Pre-Commit und CI |
| `pnpm check:store-serialization` | Verlangt, dass eine Funktion, die eine lokale Collection **liest, ändert und zurückschreibt**, den Ablauf serialisiert. Zwischen Lesen und Schreiben liegt ein echtes `await` (IndexedDB, AES-GCM): Zwei gleichzeitige Aufrufe lesen denselben Stand, und der zweite schreibt eine Fassung ohne das Element des ersten — lautlos, ohne Fehler, ohne Log. #293 hat das an einer verlorenen **Einstellung** bemerkt; nachgemessen stand dieselbe Sequenz an **27 Stellen in 12 Dateien**, und dort verliert sie eine **Buchung**. Eine verlorene Einstellung fällt beim nächsten Öffnen des Dialogs auf; eine verlorene Buchung hinterlässt keine Lücke, nach der jemand sucht, und verfälscht ab dann jede Summe. Ersatz ist `mutateLocalFinanceList(key, (items) => …)` (`@/services/local-finance-store`) bzw. `withKeyLock` (`@/lib/key-mutex`) für Kategorien, Einstellungen und den Chunk-Index. **Sechs Familien seit dem Audit 2026-09** — dazugekommen sind die **Buchungs-Chunks** und der **v3-Blob**, also ausgerechnet die Familie, um derentwillen die Regel formuliert wurde: Gesperrt war bis dahin nur der *Index*, nicht die Chunks, in denen die Buchungen liegen. Zu den Leseverben der Chunk-Familie zählt `readLegacyV3Transactions`, weil der Migrationslauf (`local-store-migrations.ts`) daraus ganze Quartale schreibt und sonst als einziger Schreiber unsichtbar bliebe. **Der Lock gehört an genau EINE Ebene:** `withKeyLock` ist nicht reentrant, und die Legacy-Schreibmethoden delegieren in ihrer ersten Zeile an ihre Chunk-Schwester — beide Ebenen auf denselben Schlüssel zu legen, lässt die äußere auf sich selbst warten und verklemmt jeden Schreibvorgang dauerhaft. **Wichtig:** Prüfungen, die eine Dublette verhindern sollen, gehören INNERHALB des Locks — davor sind sie Zierde, weil zwei gleichzeitige Aufrufe beide an ihnen vorbeikommen. Gemeldet wird die innerste Funktion; reines Lesen (`getX`) und reines Ersetzen der ganzen Liste (`replaceX`) sind ausdrücklich in Ordnung. **Ohne Ausnahmeliste** — wie `check:a11y-names`: Ein begründeter Einzelfall hiesse hier „an dieser Stelle darf gelegentlich eine Buchung verloren gehen". Die Erkennung steht in `store-serialization-core.mjs` (TypeScript-AST) und ist ohne Dateisystem testbar. Läuft in Pre-Commit und CI |
...
| `pnpm check:type-scale` | Verlangt, dass gerenderter Text mindestens **11 px** gross ist. `docs/design-principles.md` Prinzip 7 nennt Kontrast und Tap-Ziele, aber nie die Schriftgrösse — und deshalb hat nie etwas rot gemacht, was auf einem Telefon mit hoher Pixeldichte schlicht nicht mehr zu lesen ist. Nachgemessen waren es **24 Stellen**, und keine davon war Dekoration: die Beschriftungen der Bodennavigation, die Abzeichen an Kategorien, das Mitteltext-Label des Sunburst, die Beschriftungen der Finanzlandschaft bei 8 px. Also gerade die Stellen, an denen jemand etwas ABLESEN soll. Die Grenze trifft ausschliesslich den Griff UNTER die Skala: `text-xs` sind 12 px, die benannte Skala liegt vollständig darüber. Reicht der Platz dann nicht, ist **weniger Text** die Antwort, nicht kleinerer — eine Beschriftung, die niemand lesen kann, belegt den Platz trotzdem. **Die Ausnahme hängt an der POSITION, nicht an der Datei**, und das ist der Kern: Eine Tailwind-Klasse steht immer im DOM, ein JSX-Attribut `fontSize={…}` immer an einem gerenderten Element; nur die Objekt-Eigenschaft `fontSize: 8` ist mehrdeutig und wird in Papier- (`jspdf`) und WebGL-Dateien (`three`) übergangen. Die erste Fassung schloss die ganze Datei aus und lag falsch: `SankeyChart.tsx` lädt jsPDF nur für seinen Export und beschriftet daneben seine Knoten auf dem Bildschirm, `CityLabels.tsx` legt DOM-Text über die WebGL-Szene — vier echte Befunde wären stillgelegt worden, weil in derselben Datei anderswo legitim eine kleine Grösse steht (dieselbe Lehre wie bei `check:money-format`). Berechnete Grössen (`fontSize: size * 0.6`) bleiben unangetastet. **Ohne Ausnahmeliste** — wie `check:a11y-names`: Ein Eintrag hiesse „an dieser Stelle darf der Text unlesbar sein". Erkennung in `type-scale-core.mjs`, ohne Dateisystem testbar. Läuft in Pre-Commit und CI |
| `pnpm check:touch-targets` | **Ratsche** auf Bedienelemente, deren Trefferbereich ausdrücklich unter **44 px** liegt (§4). `check:platform-parity` prüft, ob eine Fläche auf schmalen Breiten ganz fehlt; der Fall daneben stand nirgends — ein Element kann vorhanden **und** unbedienbar sein, weil sein Trefferbereich kleiner ist als eine Fingerkuppe. Eine Funktion, die man mit dem Daumen nicht trifft, ist auf dem Telefon nicht vorhanden; Parität, die man nicht treffen kann, ist keine. Das Mass ist keine neue Setzung: Apple (HIG) und Google (Material, 48 dp) nennen es übereinstimmend, und die App benutzt `min-h-[44px]` an 15 Stellen bereits selbst — nur unerzwungen. **Zwei Spalten**, weil 186 der 217 Fundstellen schlicht `size="sm"` sind und die 31 per Klasse gedrückten Elemente (bis hinunter zu 20 px) sonst unter dem Rauschen verschwänden (dieselbe Lehre wie bei `slice-presentation-budget.json`): **`max`** zählt die per KLASSE verkleinerten (`h-8`, `size-8`, `h-[36px]`), **`maxVarianten`** die per VARIANTE verkleinerten (`sm`, `icon`). **Beide stehen seit der Mobil-Überarbeitung auf 0** und sind damit kein Backlog mehr, sondern ein Wächter gegen den Rückfall. Behoben wurde beides mit derselben Bauform: `fokussiert:min-h-11` **neben** die optische Grösse (bei quadratischen Zielen zusätzlich `min-w-11`) — das Icon bleibt klein, die Fläche darum nicht. **`fokussiert:` und nicht pauschal**, weil das Mass von der Fingerkuppe kommt und nicht vom Bildschirm: In der kompakten Dichte ist ein 36-px-Knopf mit der Maus präzise treffbar. Die Variante hängt am `data-density`-Attribut (`src/index.css`, `useDensityRootAttribute`), also an derselben EINEN Entscheidung wie die Wahl der Präsentation — `docs/architecture/darstellungsdichte.md` lehnt ein zweites Kriterium (eigene Media Query für grobe Zeigegeräte, Breakpoint) ausdrücklich ab. **Der Wächter liest die Variantenhöhen aus `ui/button.tsx` selbst** (`varianteHoehenAus`); vorher stand dort eine Kopie, und die machte das Versprechen „EINE Entscheidung in ui/button.tsx, danach erreicht die Zahl 0" unhaltbar: Wer die Entscheidung traf, änderte `button.tsx`, der Wächter las weiter seine Kopie und zählte unverändert 186. Eine Ratsche, die ihre eigene Behebung nicht bemerken kann, misst nichts. **Benannte Grenze:** `<Button>` ohne Grössenangabe ist `h-10` und damit ebenfalls 40 px — nicht gezählt, weil das EINE Entscheidung in `ui/button.tsx` ist und nicht dreihundert an den Aufrufstellen; sie mitzuzählen hätte jeden Knopf der App zum Einzelbefund gemacht und genau die Stellen verdeckt, an denen wirklich jemand verkleinert hat. Erkennung in `touch-target-core.mjs`. Läuft in Pre-Commit und CI |
...
| `pnpm check:card-rule` | Zwei Prüfungen in einer. **(1) Harte Regel:** Karten-Chrome ohne Interaktions-Signal (§9 „Karten sind Aktionen"). Ausnahmeliste `card-rule-allowlist.json`, heute leer. **(2) Ratsche seit der Mobil-Überarbeitung** (`card-rule-budget.json`), weil die harte Regel je DATEI fragt und damit den häufigsten Fall nicht sieht: Eine Karte voller anklickbarer ZEILEN trägt immer irgendein Signal — und ist trotzdem die tote Schachtel, die Prinzip 8 verbietet. Gemessen auf der Übersicht umschloss `<Card>` die letzten Buchungen; angeklickt wurden die Zeilen darin. Ob eine Karte „als Ganzes" klickbar ist, bleibt statisch unentscheidbar — die **Menge** ist es nicht: **`max`** zählt Kartenrahmen im ganzen Baum (Stand 149; `InteractiveCard` zählt NICHT mit, sie ist die Bauform, die das Versprechen einlöst), **`maxFokussiert`** zählt Boxen unter `features/*/presentation/mobile/`, wo ADR Regel 9 sie ganz verbietet (Stand 2, Ziel 0). Eine Haarlinie (`border-t`) ist erlaubt, ein Knopf mit Rundung und Rahmen ist ein Knopf. Beide Zahlen dürfen nur sinken. Läuft in Pre-Commit und CI |
...
### Anpassen, nicht amputieren

Feature-Parität heißt **umbauen**, nicht weglassen. Eine Funktion verschwindet
nicht, weil sie auf dem Telefon unbequem ist — dann wird ihre Interaktion neu
gedacht. Vier Dimensionen, in dieser Reihenfolge zu prüfen; die ersten beiden
sind maschinell abgesichert, die letzten beiden Sache des Selbst-Reviews:

| Dimension | Was zu tun ist | Wächter |
|---|---|---|
| **Layout** | Mehrspaltiges wird einspaltig, Wertebereiche (`clamp`) werden nachgezogen, ein neuer Breakpoint nur bei einem echten Gestaltungsbruch | `check:platform-parity` (Fläche fehlt auf schmalen Breiten ganz) |
| **Tippziele** | Mindestens **44 px** Trefferbereich, ausreichend Abstand zwischen benachbarten Zielen. Die Tap-Zone darf **grösser** sein als das Bild — `min-h-[44px] min-w-[44px]` neben ein kleines Icon ist die Regelform, nicht ein grösseres Icon | `check:touch-targets` (Ratsche) |
| **Navigation** | Seitenleiste → Bodennavigation oder Slide-out, dichte Werkzeugleiste → Menü, **jeder Hover-Zustand braucht ein Touch-Äquivalent** — was nur bei Hover erscheint, existiert auf dem Telefon nicht | Selbst-Review |
| **Inhaltsrang** | Je Kontext entscheiden, was sichtbar, was eingeklappt und was hinter einem Schritt liegt. Eingeklappt ist erlaubt, **entfernt nicht** | Selbst-Review |

Der häufigste Fehler ist nicht die fehlende Fläche, sondern **Mobile als
kleinerer Desktop**: dieselbe Anordnung, nur enger, mit denselben Hover-Wegen
und denselben Trefferbereichen. Gemessen war genau das der Bestand — 217
Bedienelemente unter dem Mindestmass, davon 31 ausdrücklich per Klasse
verkleinert.
...
- **Text unter 11 px ist kein Text.** Die benannte Skala beginnt bei `text-xs`
  (12 px) und liegt damit vollständig über der Grenze; ein Willkürwert darunter
  (`text-[10px]`, `text-[8px]`) ist immer der Griff unter die Skala. Reicht der
  Platz nicht, ist **weniger Text** die Antwort, nicht kleinerer — eine
  Beschriftung, die niemand lesen kann, belegt den Platz trotzdem. Erzwungen
  durch `pnpm check:type-scale` (§2).
- **Ein Trefferbereich ist mindestens 44 px, das Bild darin darf kleiner sein.**
  Ein 16-px-Icon in einer 44-px-Fläche ist richtig; ein 32-px-Knopf ist es
  nicht, auch wenn er auf dem Desktop gut aussieht. Ratsche:
  `pnpm check:touch-targets` (§2), Zusammenhang: §4 „Anpassen, nicht
  amputieren".
...
(`pnpm check:store-serialization`), die stillen Kappungen auf Buchungen
(`pnpm check:transaction-limits`), die Wirksamkeit des Sanften Modus
...
Trennung von Ansicht und Daten (`pnpm check:view-data`), die Lesbarkeit des
Bildschirmtexts (`pnpm check:type-scale`), die Mindestgrösse der Tippziele
(`pnpm check:touch-targets`), die Slice-
