...
| `pnpm check:touch-targets` | **Ratsche** auf Bedienelemente, deren Trefferbereich ausdrücklich unter **44 px** liegt (§4). `check:platform-parity` prüft, ob eine Fläche auf schmalen Breiten ganz fehlt; der Fall daneben stand nirgends — ein Element kann vorhanden **und** unbedienbar sein, weil sein Trefferbereich kleiner ist als eine Fingerkuppe. Eine Funktion, die man mit dem Daumen nicht trifft, ist auf dem Telefon nicht vorhanden; Parität, die man nicht treffen kann, ist keine. Das Mass ist keine neue Setzung: Apple (HIG) und Google (Material, 48 dp) nennen es übereinstimmend, und die App benutzt `min-h-[44px]` an 15 Stellen bereits selbst — nur unerzwungen. **Zwei Spalten**, weil 186 der 217 Fundstellen schlicht `size="sm"` sind und die 31 per Klasse gedrückten Elemente (bis hinunter zu 20 px) sonst unter dem Rauschen verschwänden (dieselbe Lehre wie bei `slice-presentation-budget.json`): **`max`** zählt die per KLASSE verkleinerten (`h-8`, `size-8`, `h-[36px]`), **`maxVarianten`** die per VARIANTE verkleinerten (`sm`, `icon`). **Beide stehen seit der Mobil-Überarbeitung auf 0** und sind damit kein Backlog mehr, sondern ein Wächter gegen den Rückfall. Behoben wurde beides mit derselben Bauform: `pointer-coarse:min-h-11` **neben** die optische Grösse (bei quadratischen Zielen zusätzlich `min-w-11`) — das Icon bleibt klein, die Fläche darum nicht. **`pointer-coarse:` und nicht pauschal**, weil das Mass von der Fingerkuppe kommt und nicht vom Bildschirm: Mit einer Maus ist ein 36-px-Knopf präzise treffbar, und Dichte ist auf dem Desktop ein Vorteil. Ein Breakpoint wäre die falsche Frage — er misst Breite, nicht Eingabeart. **Der Wächter liest die Variantenhöhen aus `ui/button.tsx` selbst** (`varianteHoehenAus`); vorher stand dort eine Kopie, und die machte das Versprechen „EINE Entscheidung in ui/button.tsx, danach erreicht die Zahl 0" unhaltbar: Wer die Entscheidung traf, änderte `button.tsx`, der Wächter las weiter seine Kopie und zählte unverändert 186. Eine Ratsche, die ihre eigene Behebung nicht bemerken kann, misst nichts. **Benannte Grenze:** `<Button>` ohne Grössenangabe ist `h-10` und damit ebenfalls 40 px — nicht gezählt, weil das EINE Entscheidung in `ui/button.tsx` ist und nicht dreihundert an den Aufrufstellen; sie mitzuzählen hätte jeden Knopf der App zum Einzelbefund gemacht und genau die Stellen verdeckt, an denen wirklich jemand verkleinert hat. Erkennung in `touch-target-core.mjs`. Läuft in Pre-Commit und CI |
